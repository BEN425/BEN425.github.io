...
function SupportCardItem({ card, onClick }) {
	...
	const [uuid, title, rarity, type] = [
		card.uuid,
		card.name,
		card.rarity,
		mylib.translate(card.type),
	];
	const src = "/images/card/" + card.id.replace(" thumb ", "_thumb_") + ".png";

	return (
		<Link href={`/support_card/${uuid}`}>
			<div
				className={styles.support_card_item}
				onClick={onClick}>
				<img
					src={src}
					className={styles.support_card_image}></img>
				<div className={styles.support_card_sub}>
					...
					<div className={styles.support_card_sub_sub}>
						<div
							className={styles.support_card_type}
							style={{ backgroundImage: `url(/icon/${type}.png)` }}></div>
						<div>{rarity}</div>
					</div>
				</div>
			</div>
...
function generateTypeButtons(typeFilter, setTypeFilter) {
	return typeList.map((value, index) => (
		<ToggleButton
			...
			onToggle={() => {
				mylib.toggleFilterList(typeFilter, setTypeFilter, index);
			}}></ToggleButton>
	));
}
