...
export default function SingleChara() {
...
function Page({ chara, skills, races }) {
	return (
		<div className={styles.main_column}>
			...
			<div className={styles.title_row}>
				{/* Character image */}
				<img
					src={"/images/chara/" + chara.id + ".png"}
					className={styles.chara_image}></img>
				{/* Title Column */}
			...
			<div className={styles.basic_row}>
				...
				<div className={styles.subcol1}>
					<div className="section middle">
						...
						<TalentGrid
							talent={chara.talent}
							growing_rate={chara.growing_rate}></TalentGrid>
					</div>
					<div className="section middle">
						<div>專屬別稱</div>
						<hr></hr>
			...
			<div className={styles.basic_row}>
				...
				<div className="section middle">
					...
					<div>
						<TargetCol
							targets={chara.targets}
							raceList={races}></TargetCol>
					</div>
				</div>
			</div>
		</div>
...
function TalentGrid({ talent, growing_rate }) {
	...
	const type_table = ["speed", "stamina", "power", "guts", "wisdom"];

	return (
		<>
			<div className={styles.talent_grid}>
				<div>場地</div>
				{talent.slice(0, 2).map((item, index) => (
				...
				<div></div>
				<div>距離</div>
				{talent.slice(2, 6).map((item, index) => (
					<div style={{ color: `var(--${item}-color)` }}>
						{`${talent_table[index + 2]} ${item}`}
					</div>
				))}
				<div>跑法</div>
				{talent.slice(6, 10).map((item, index) => (
					<div style={{ color: `var(--${item}-color)` }}>
			...
			<div className={styles.grow_grid}>
				<div>成長率</div>
				{growing_rate.map((item, index) => (
					<div style={{ color: `var(--${type_table[index]}-color)` }}>{item}</div>
				))}
			</div>
		</>
	);
...
function SkillCol({ skills, skillList }) {
	return (
		<div className={styles.skill_col}>
			<div>固有技能</div>
			<mylib.SkillBlock
				skill={skills.special}
				skillList={skillList}></mylib.SkillBlock>
			<div>初始技能</div>
			<mylib.SkillCol
				skills={skills.initial}
				skillList={skillList}></mylib.SkillCol>
			<div>覺醒 Lv2</div>
			<mylib.SkillBlock
				skill={skills.lv2}
				skillList={skillList}></mylib.SkillBlock>
...
function TargetCol({ targets, raceList }) {
	function TargetBlock({ target }) {
		// Find the race data related to the target

		var raceData = undefined;
		raceData = raceList.G1.find(race => target.title.includes(race.abbr || race.basic.name));
		if (raceData === undefined)
			raceData = raceList.G2.find(race =>
				target.title.includes(race.abbr || race.basic.name)
			);
		if (raceData === undefined)
			raceData = raceList.G3.find(race =>
				target.title.includes(race.abbr || race.basic.name)
			);
		if (raceData === undefined)
			raceData = raceList.OP.find(race =>
				target.title.includes(race.abbr || race.basic.name)
			);
		if (raceData === undefined)
			raceData = raceList.Pre.find(race =>
				target.title.includes(race.abbr || race.basic.name)
			);
		...
		return (
			<div className="target_block">
				{raceData === undefined ? (
					...
					<Link href={`/race/${raceData.uuid}`}>{target.title}</Link>
				)}
				<div>{target.date}</div>
			</div>
		);
	...
	return (
		<>
			{targets.map(element => (
				<TargetBlock target={element}></TargetBlock>
			))}
		</>
	);
}
